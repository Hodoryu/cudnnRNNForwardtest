...
"""
Debug script to understand bidirectional RNN output format
"""
...
def debug_bidirectional_format():
    """Debug the bidirectional RNN output format"""
    print("=== Debug Bidirectional RNN Output Format ===")
...
"""
    ...
    # Write the debug code
    with open('debug_bidirectional.cpp', 'w') as f:
        f.write(debug_code)
    
    # Compile the debug test
    compile_cmd = ['g++', '-std=c++17', '-I../', '-I/usr/local/cuda/include', 
                   ...
                   '-lonnxruntime', '-o', 'debug_bidirectional', 'debug_bidirectional.cpp', 
    ...
    try:
        ...
        # Run the debug test
        test_result = subprocess.run(['./debug_bidirectional'], capture_output=True, text=True, timeout=30)
...
def main():
    """Main function"""
    print("Bidirectional RNN Format Debug")
    print("=" * 50)
    
    success = debug_bidirectional_format()
    
    if success:
        print("\\n🎉 DEBUG COMPLETE")
        print("Check the output files for format analysis.")
    else:
        print("\\n❌ DEBUG FAILED")
